...
class heap:
        ...
        if self.count==0:
            ...
            while child<=self.count:
                if child+1<=self.count: # 순서가 꼬였어도 자식중에 큰놈이랑
                    ...
                    if abs(self.arr[child+1])<abs(self.arr[child]):
                        child+=1
                    elif abs(self.arr[child+1])==abs(self.arr[child]):
                        if self.arr[child+1]<self.arr[child]:
                            child+=1
                if abs(temp)<abs(self.arr[child]):
                    break
                elif abs(temp)==abs(self.arr[child]):
    ...
    def insert(self,num):
        ...
        while i//2>0:
            if abs(self.arr[i//2])>abs(num):
                self.arr[i] = self.arr[i//2]
                i//=2
            elif abs(self.arr[i//2])==abs(num):
                if self.arr[i//2]>num:
                    self.arr[i]=self.arr[i//2]
                    i//=2
                else: break
